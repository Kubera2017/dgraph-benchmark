	...
	for i := 0; i < batchCount; i++ {
		...
		for j := 0; j < batchSize; j++ {
			...
			m11 := &api.Mutation{
				...
				SetNquads: []byte(`uid(uri_from` + strconv.Itoa(j) + `) <BACKLINK> uid(uri_to` + strconv.Itoa(j) + `) .
				`),
			}
			...
			m01 := &api.Mutation{
				...
				SetNquads: []byte(`_:newUriFrom` + strconv.Itoa(j) + ` <BACKLINK> uid(uri_to` + strconv.Itoa(j) + `) .
				`),
			}
			...
			m10 := &api.Mutation{
				...
				SetNquads: []byte(`uid(uri_from` + strconv.Itoa(j) + `) <BACKLINK> _:newUriTo` + strconv.Itoa(j) + ` .
				`),
			}
			...
			m00 := &api.Mutation{
				...
				SetNquads: []byte(`_:newUriFrom` + strconv.Itoa(j) + ` <BACKLINK> _:newUriTo` + strconv.Itoa(j) + ` .
				`),
			}
